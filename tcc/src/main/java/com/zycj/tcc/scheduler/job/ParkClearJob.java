 ...
 */
@Component
public class ParkClearJob {
	private final static Logger log = Logger.getLogger(ParkClearJob.class);
	...
	public void clear(){
		log.info(DateUtil.format_YMDHMSx(new Date())+" 开始---->执行日终清算="+SystemConfig.Park_Clear_Job_CronExpression);
...
//		updateFeeRegularCache();//not today
		...
		log.info(DateUtil.format_YMDHMSx(new Date())+" 结束---->执行日终清算="+SystemConfig.Park_Clear_Job_CronExpression);
	}
...
//		List<OrderInfo> oiList=orderInfoDao.selectParkingOrderByToday();
//		log.warn("需要日终驶出订单数为："+oiList.size());
//		int todayOrderClearCount=clearOrder(oiList);
//		log.warn("清算的日终驶出订单数为："+todayOrderClearCount);
		//历史未完成订单
		OrderInfoExample oie=new OrderInfoExample();
		oie.or().andOrderStatusEqualTo(OrderStatus.IN.getIndex());
		oiList=orderInfoMapper.selectByExample(oie);
		log.warn("统计的历史未驶出订单数为："+oiList.size());
		int historyOrderClearCount=clearOrder(oiList);
		log.warn("清算历史未驶出的订单数为："+historyOrderClearCount);
		long endTime=System.currentTimeMillis(); //执行结束时间
		log.warn("》》》》》》》》结束  清算订单.... 用时："+DateUtil.getSFM(endTime-startTime));
	}
	//更新车位是停车状态的为空闲
