...
public class MqttServer {
	...
	public static void main(String[] args) {
		try {
			...
			connection.connect();
//			Thread.sleep(5000);
//			connection.publish("foo1", "Hello foo1".getBytes(), QoS.AT_LEAST_ONCE, false);
			Topic[] topics = {new Topic("server", QoS.AT_LEAST_ONCE)};
			connection.subscribe(topics);
			while(true){
				Message message = connection.receive();
				String msg = new String(message.getPayload());
				System.out.println(message.getTopic() + "===" + msg);
				message.ack();
				msg = msg.substring(0,4);
//				connection.publish(msg, (msg + " 你好").getBytes(), QoS.AT_LEAST_ONCE, true);
			}
		} catch (Exception e) {
			LOG.error("mqtt获得连接配置出现了异常！", e);
		}
	}
}
